        ...
        yepnope.showLoading = function (n) {
            ...
            $(".loader").animate({minWidth: Math.round(yepnope.percent)+"px"}, {
                ...
                progress: function () {
                    ...
                    if (!isWebkitBlurSupported && current % 20 === 0) $cc.css({filter: "url('data:image/svg+xml;utf8,<svg xmlns=\"http://www.w3.org/2000/svg\"><filter id=\"blur-overlay\"><feGaussianBlur stdDeviation=\"" + (maxBlur - maxBlur/(steps+1)*n) + "\"/></filter></svg>#blur-overlay')"});
                    if (current === 100) {
